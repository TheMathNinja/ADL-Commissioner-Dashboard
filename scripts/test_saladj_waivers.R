...
stopifnot(saladj_waiver_note(old,"pending",now)=="Manual; ON WAIVERS CURRENTLY - MFL still lists player after expected 5 a.m. ET run on Sep 16, 2026; CHECK SALARY")
future_clear <- as.POSIXct("2026-09-18 09:00:00", tz="UTC")
stopifnot(saladj_waiver_note("", "pending", future_clear, now)=="ON WAIVERS CURRENTLY - expected to clear at 5 a.m. ET on Sep 18, 2026")
stopifnot(saladj_waiver_note("CHECK SALARY","unknown",future_clear,now)=="WAIVER STATUS UNVERIFIED - expected 5 a.m. ET run on Sep 18, 2026; check MFL; CHECK SALARY")
stopifnot(saladj_waiver_note("", "not_listed", now, now)=="NOT LISTED ON MFL WAIVERS AT LAST CHECK")
